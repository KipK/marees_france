...
async def _async_fetch_with_retry(
    ...
    _LOGGER.debug("Marées France: Preparing to fetch %s for %s from %s", data_type, harbor_id, url)
    ...
    for attempt in range(max_retries):
        ...
        try:
            async with asyncio.timeout(timeout):
                ...
                # Check for specific non-retryable errors first if needed (e.g., 401, 403, 404)
                # if response.status in [401, 403, 404]:
                #     _LOGGER.error(...)
                #     return None # Or raise specific exception
                response.raise_for_status() # Raises ClientResponseError for 4xx/5xx
                data = await response.json()
                _LOGGER.debug("Marées France: Successfully fetched %s for %s (attempt %d/%d)", data_type, harbor_id, attempt + 1, max_retries)
                return data # Success

        except asyncio.TimeoutError:
            _LOGGER.warning(
                "Marées France: Timeout fetching %s for %s (attempt %d/%d). Retrying in %ds...",
                data_type, harbor_id, attempt + 1, max_retries, current_delay
            )
        except aiohttp.ClientResponseError as err:
             _LOGGER.warning(
                "Marées France: HTTP error %s fetching %s for %s (attempt %d/%d): %s. Retrying in %ds...",
                err.status, data_type, harbor_id, attempt + 1, max_retries, err.message, current_delay
            )
        except aiohttp.ClientError as err:
            _LOGGER.warning(
                "Marées France: Client error fetching %s for %s (attempt %d/%d): %s. Retrying in %ds...",
             ...
             _LOGGER.warning(
                "Marées France: Unexpected error fetching %s for %s (attempt %d/%d): %s. Retrying in %ds...",
                data_type, harbor_id, attempt + 1, max_retries, err, current_delay
            )
        ...
        if attempt < max_retries - 1:
            ...
            # Last attempt failed
            _LOGGER.error(
                "Marées France: Failed to fetch %s for %s after %d attempts. Please check network/configuration. Will retry on next scheduled update.",
                data_type, harbor_id, max_retries
            )
            # The "schedule retry in 6 hours" is implicitly handled by the daily prefetch schedule.
            # We log a clear error here. The calling function should handle the None return value.
...
async def _async_fetch_and_store_water_level(
    ...
    try:
        ...
        _LOGGER.exception(
            "Marées France: Unexpected error saving water level cache for %s on %s",
            harbor_name,
            date_str,
        )
...
async def _async_fetch_and_store_tides(
    ...
    if fetched_data_dict is None or not isinstance(fetched_data_dict, dict):
        ...
        _LOGGER.error("Marées France: Failed to fetch or received invalid format for tide data for %s starting %s.", harbor_id, start_date_str)
    ...
    try:
        ...
        _LOGGER.exception(
            "Marées France: Unexpected error saving tides cache for %s starting %s",
            harbor_id,
            start_date_str,
        )
...
async def _async_fetch_and_store_coefficients(
    ...
    if fetched_data_list is None or not isinstance(fetched_data_list, list):
        # Error logged by helper or unexpected format
        _LOGGER.error("Marées France: Failed to fetch or received invalid format for coefficient data for %s starting %s (%d days).", harbor_id, start_date_str, days)
        return False # Indicate failure
    ...
    try:
        ...
        _LOGGER.exception(
            "Marées France: Unexpected error saving coefficients cache for %s starting %s (%d days)",
            harbor_id,
            start_date_str,
            days
        )
