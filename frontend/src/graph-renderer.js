...
export class GraphRenderer {
    ...
    _heightToY(h) {
        if (!this.graphMargin || this.graphHeight === null || this.yDomainMin === null || !this.yRange) return 0; // Guard
        return this.graphMargin.top + this.graphHeight - ((h - this.yDomainMin) / this.yRange) * this.graphHeight;
    }

    _xToTotalMinutes(x) {
        if (!this.graphMargin || this.graphWidth === null || this.graphWidth <= 0) return 0; // Guard
    ...
    _interpolateHeight(targetTotalMinutes) {
        ...
        for (let i = 0; i < this.pointsData.length; i++) {
            ...
            if (this.pointsData[i].totalMinutes > targetTotalMinutes) { nextPoint = this.pointsData[i]; break; }
        }
    ...
    _updateElementScale() {
        if (!this.svgContainer || !this.svgDraw || this.elementsToKeepSize.length === 0) {
            return;
        }
        ...
        this.elementsToKeepSize = this.elementsToKeepSize.filter(element =>
    ...
    drawGraph(tideData, waterLevels, selectedDay) {
        ...
        if (!tideData || tideData.error || !tideData.response) {
            ...
            const errorText = this.svgDraw.text(errorMessage)
        ...
        if (!Array.isArray(levelsData) || levelsData.length === 0) {
            const noDataText = this.svgDraw.text(localizeCard('ui.card.marees_france.no_data_for_day', this.hass))
                .move(viewBoxWidth / 2, viewBoxHeight / 2)
        ...
        this.pointsData = levelsData.map(item => {
        ...
        if (this.pointsData.length < 2) {
             const notEnoughDataText = this.svgDraw.text(localizeCard('ui.card.marees_france.no_data_for_day', this.hass))
                 .move(viewBoxWidth / 2, viewBoxHeight / 2)
                 .font({ fill: 'var(--secondary-text-color, grey)', size: 14, anchor: 'middle' });
       ...
       const pathData = this.pointsData.map((p, index) => {
       ...
       const lastPointX = this._timeToX(this.pointsData[this.pointsData.length - 1].totalMinutes);
       ...
       if (Array.isArray(tideEventsForDay)) {
           tideEventsForDay.forEach(tideArr => {
               ...
               tideMarkers.push({ x, y, time, height, coefficient: isHigh ? coefficient : null, isHigh });
           });
       }
       ...
       if (selectedDay === now.toISOString().slice(0, 10)) {
        ...
        const coefBoxBorderColor = 'var(--ha-card-border-color, var(--divider-color, grey))';
        ...
        xTicks.forEach(tick => {
             if (tick.label) {
                const textEl = draw.text(tick.label)
        ...
        tideMarkers.forEach(marker => {
            ...
            if (marker.isHigh && marker.coefficient) {
                ...
                const tempText = draw.text(coefText).font({ size: coefFontSize, weight: 'bold', anchor: 'middle' }).attr('dominant-baseline', 'central').opacity(0);
                ...
                coefGroup.rect(boxWidth, boxHeight)
                ...
                const coefColor = coefValue >= 100 ? 'var(--warning-color)' : primaryTextColor;
                coefGroup.text(coefText)
                ...
                if (lineEndY > lineStartY) {
                     coefGroup.line(marker.x, lineStartY, marker.x, lineEndY)
            ...
            arrowGroup.text(marker.time)
                .font({ fill: arrowAndTextColor, size: tideTimeFontSize, weight: 'bold' })
                .attr('text-anchor', 'middle')
                .cx(marker.x).y(timeTextY);

            arrowGroup.text(`${marker.height.toFixed(1)}m`)
                .font({ fill: arrowAndTextColor, size: tideHeightFontSize })
                .attr('text-anchor', 'middle')
                .cx(marker.x).y(heightTextY);
        ...
        if (currentTimeMarker) {
            ...
            const dotCircle = dotGroup.circle(dotRadius * 2)
                .center(currentTimeMarker.x, currentTimeMarker.y)
            ...
            hitAreaCircle.node.addEventListener('mouseover', (e) => this.card._handleTooltipShow(e));
            hitAreaCircle.node.addEventListener('mouseout', () => this.card._handleTooltipHide());
            hitAreaCircle.node.addEventListener('mousedown', (e) => this.card._handleDragStart(e, dotGroup, dotCircle, hitAreaCircle));
            hitAreaCircle.node.addEventListener('touchstart', (e) => this.card._handleDragStart(e, dotGroup, dotCircle, hitAreaCircle), { passive: false });
        }
